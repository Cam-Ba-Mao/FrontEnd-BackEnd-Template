...
function iknow_get_home_posts($category = null) {
	$defaults = array(
		'orderby'    => 'name',
		'order'      => 'ASC',
		'hide_empty' => 1,
	...
	if(!is_null($category)) {
		$categories = wp_list_filter( $categories, array( 'parent' => $category ) );
	}
	else {
	...
	if( is_front_page() ) {
		foreach ( $categories as $cat ) {
			...
			$out .= '<div class="panel ' . $bg_class . '">';
			$out .= '<div class="panel-heading level is-mobile">';
			$out .= '<div class="level-left">';
			$out .= '<div class="level-item"><span class="' . esc_attr( $cat_icon ) . ' ' . $color_class . '"></span></div>';
			$out .= '<div class="level-item"><a class="' . $color_class . '" href="' . esc_url(get_category_link($cat->cat_ID)) . '" title="' . esc_attr($cat->name) . '">' . esc_attr( $cat->name ) . '</a></div>';
			$out .= '</div>';
			$out .= '</div>';
			$out .= '</div>';
			$out .= '</div>';
		...
		foreach ( $categories as $cat ) {
			...
			$out .= '<div class="level-item"><a class="' . $color_class . '" href="' . esc_url(get_category_link($cat->cat_ID)) . '" title="' . esc_attr($cat->name) . '">' . esc_attr( $cat->name ) . '</a></div>';
			$out .= '</div>';
//			$out .= '<div class="level-right has-text-white">' . absint( $cat->count ) . '</div>';
			$out .= '</div>';
